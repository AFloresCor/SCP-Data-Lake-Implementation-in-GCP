...
|----|----|----|
| **Executives & Sponsors** | Strategic value, outcomes, cost, and risk | Executive Summary, Project Background, Solution Overview, Cost Analysis, Roadmap |
| **Project & Product Managers** | Scope, timeline, key decisions, trade-offs | Project Background, Solution Overview, Architecture, Roadmap |
| **Data Analysts & Scientists** | Data availability, quality, access patterns, tooling | Solution Overview, Data Lake Design, GCP Services, Recommendations |
| **Data & Cloud Engineers** | Technical architecture, pipelines, configuration, code | Architecture, GCP Services, Data Lake Design, Implementation, Security & Compliance |
| **Security & Compliance Officers** | Data governance, access control, encryption, audit | Security & Compliance, Recommendations |
...
![briefcase](diagrams/icons/briefcase.png) **Business Overview** sections provide concise, non-technical summaries.
...
![cogs](diagrams/icons/cogs.png) **Technical Detail** sections contain in-depth specifics for engineers and architects.
...
![lightbulb](diagrams/icons/lightbulb.png) **Recommendation** sections highlight key decisions and guidance that can be acted upon immediately.
...
|---------|------------|-----------------------|--------------------|
| 1.0     | March 2026 | Data Engineering Team | Initial release    |
...
![briefcase](diagrams/icons/briefcase.png) Business Overview

</div>

Our data was scattered across dozens of isolated systems — departmental databases, third-party SaaS exports, and manual spreadsheets. Analysts spent up to **60% of their time** just locating and preparing data instead of generating insights. Leadership lacked a single, reliable source of truth for key business metrics.

Aside from that, a new project was on the horizon that would require integrating data from hundreds of new sources. The existing data landscape was not only inefficient but also a barrier to innovation.

</div>

Before this project, the organization faced three critical data challenges:

1.  **Data Silos** — Each business unit maintained its own data store with no cross-team visibility or shared governance.

2.  **Slow Time-to-Insight** — Producing a consolidated report required days of manual data extraction and reconciliation.

3.  **Low innovation** — The fragmented data landscape made it difficult to implement data-driven projects, especially those involving machine learning.
...
| Outcome | Result |
|----|----|
| Unified Data Platform | 100% of priority data sources ingested and catalogued in a single environment. |
| Faster Insights | Report generation time reduced from days to minutes. |
| Data Quality | Automated quality checks catch and quarantine bad data before it reaches consumers. |
| Self-Service Analytics | Analysts and data scientists can explore curated data without waiting on engineering. |
| Governance & Compliance | Centralized access control, lineage tracking, and audit logs across all datasets. |
...
This platform is not just a storage system — it is a **strategic data asset** that accelerates every team that depends on data. It is the basis on which **"Anonymous Company"** can build complex data projects, ride the wave of AI, and maintain a competitive edge in the market.
...
| Core Services | Cloud Storage, BigQuery, Dataflow, Dataform, Composer, Pub/Sub, Cloud Run, Dataplex, IAM |
| Team | 2 Data Engineers, 1 Cloud Architect, 1 Data Analyst |
| Timeline | 6 months (planning through production go-live) |
| Estimated Annual Cloud Cost | *See [Cost Analysis](#cost-analysis) for a detailed breakdown.* |
...
This initiative was launched to solve a problem that was limiting the organization’s ability to grow and compete: data was abundant but nearly impossible to use effectively. This chapter describes where things stood before the project, and what drove the decision to invest in a centralized data platform.
...
|----|----|----|----|----|
| CRM Platform | Internal | API and CSV exports | Sales Team | *1M records* |
| Finance System | Internal | API and CSV exports | Finance Team | *5 years of transactions* |
| Marketing Analytics | Internal | CSV exports | Marketing Team | *Mixed, inconsistent* |
| Web Scraping Data | External | JSON files, HTML | Data Engineering Team | *no data, new sources anticipated* |
| Third-Party Data Feeds | External | CSV and JSON | Data Engineering Team | *no data, new sources anticipated* |

### Key Pain Points

> [!NOTE]
> The following pain points were identified through stakeholder interviews and a 2-week discovery phase conducted in Q1 2024. Each maps to a specific architectural decision described in [Architecture](#architecture).

1.  **No Single Source of Truth**

    Each team maintained its own version of core metrics, leading to conflicting numbers in leadership meetings.

2.  **Manual and Error-Prone Pipelines**
...
| Driver | Description |
|----|----|
| **Strategic Growth** | Leadership identified data-driven decision-making as a core pillar of the 2024–2025 strategy. |
| **Regulatory Compliance** | New internal data governance policies required centralized audit trails and access controls. |
| **Ambitions** | High-impact projects were on hold due to the lack of a reliable data foundation. |
| **Engineering Bandwidth** | The data engineering team spent the majority of its time on data wrangling rather than building revenue-generating features. |
...
|----|----|----|
| Discovery & Design | Month 1–2 | Architecture design, tool selection, stakeholder alignment, infrastructure provisioning. |
| Core Infrastructure | Month 2–3 | GCP project setup, networking, IAM, Cloud Storage buckets, BigQuery datasets. |
| Ingestion Pipelines | Month 3–4 | Dataflow pipelines for each priority source system; Bronze layer populated. |
| Transformation Layer | Month 4–5 | Silver and Gold layer transformations; data quality checks; Dataplex catalogue. |
| Analytics & Handoff | Month 5–6 | Self-service layer, dashboards, documentation, team training, go-live. |

## Stakeholders

| Name / Role | Organization | Responsibility |
|----|----|----|
| *Executive Sponsor* | Leadership | Strategic alignment and budget approval. |
| *Data Engineering Lead* | Technology | Technical design and delivery ownership. |
| *Cloud Architect* | Technology | GCP architecture validation and security review. |
| *Business Analyst* | Operations | Requirements gathering and acceptance testing. |
...
Think of the data lake as a **central library for all company data**. Every data source feeds into it, data is cleaned and organized automatically, and anyone with the right permissions can query it instantly — without waiting for engineering. Built on Google Cloud, it scales automatically and only charges for what you use.
...
|----|----|----|
| ![circle](diagrams/icons/circle.png) **Bronze** | Raw / Landing | Stores raw, unmodified data exactly as received from source systems. Acts as an immutable archive. Nothing is ever deleted from Bronze. |
| ![adjust](diagrams/icons/adjust.png) **Silver** | Cleansed / Conformed | Data is validated, deduplicated, standardized, and enriched. This is the single source of truth for analytical teams. |
| ![circle](diagrams/icons/circle.png) **Gold** | Curated / Serving | Purpose-built, aggregated datasets optimized for specific use cases — dashboards, reports, ML training sets, or API serving. |
...
Google Cloud was selected because it offers mature serverless data processing services and the best integration with our existing GCP footprint, with BigQuery being the gold standard for large-scale analytical queries. We can run complex company-wide reports in seconds for a fraction of a cent.
...
|----|----|----|----|
| BigQuery (serverless analytics) | ![check](diagrams/icons/check.png) Native | ![times](diagrams/icons/times.png) Redshift (cluster-based) | ![times](diagrams/icons/times.png) Synapse (complex) |
| Fully managed streaming ingest | ![check](diagrams/icons/check.png) Pub/Sub + Dataflow | ![check](diagrams/icons/check.png) Kinesis + Glue | ![check](diagrams/icons/check.png) Event Hubs + ADF |
| Data cataloguing & governance | ![check](diagrams/icons/check.png) Dataplex + Data Catalog | ![check](diagrams/icons/check.png) Glue Catalog | ![check](diagrams/icons/check.png) Purview |
| Cost model | ![check](diagrams/icons/check.png) Pay-per-query | ![minus](diagrams/icons/minus.png) Cluster uptime | ![minus](diagrams/icons/minus.png) DTU/vCore |
| Existing GCP footprint | ![check](diagrams/icons/check.png) Yes | ![times](diagrams/icons/times.png) No | ![times](diagrams/icons/times.png) No |
...
|----|----|----|
| **Cloud Storage (GCS)** | Object storage for unstructured Bronze layer data. | Bronze |
| **Cloud Pub/Sub** | Managed message queue for ingesting scraping events. | Bronze |
| **Cloud Run** | Serverless container execution for web scraping and API-based ingestion. | Bronze |
| **Cloud Dataflow** | Fully managed Apache Beam pipelines for batch and streaming transformations. | Silver |
| **BigQuery** | Serverless analytical data warehouse for storing structured Bronze layer data, but also powering the Silver and Gold layers and all BI workloads. | Bronze, Silver, Gold |
| **Dataplex** | Unified data governance: cataloguing, lineage, quality checks, and policy enforcement. | Silver, Gold |
| **Cloud Composer (Airflow)** | Workflow orchestration for scheduling batch pipelines. | Bronze, Silver |
| **Dataform (dbt)** | SQL-based transformations and testing framework for data modeling. | Silver, Gold |
| **IAM & VPC Service Controls** | Identity and access management and network controls. | Bronze, Silver, Gold |
| **Looker Studio / Looker** | Business intelligence and self-service reporting for end users. | Gold |
...
This chapter describes **how all the pieces fit together**. The architecture ensures data flows reliably from every source system into the data lake, is automatically cleaned, and becomes available for analysis — all without manual intervention.
...
plantuml::diagrams/architecture.puml\[GCP Data Lake Architecture,align="center",title-align="center",format=svg\]
...
|----|----|
| **Immutability** | Bronze layer data is never modified or deleted. Transformations produce new datasets in Silver and Gold. |
| **Idempotency** | All pipelines can be safely re-run without producing duplicate data. |
| **Least Privilege** | Every service account and user role has only the minimum permissions required. |
| **Fail Fast, Fail Visibly** | Pipeline failures trigger immediate alerts. Bad data is quarantined, not silently discarded. |
| **Scalability** | Serverless services (Dataflow, BigQuery) automatically scale with data volume and concurrency. |

## Ingestion Architecture

The ingestion pipeline acquires data from all source systems and lands it, unmodified, in the Bronze layer. Cloud Composer orchestrates each run; Cloud Run and Cloud Dataflow communicate exclusively through Cloud Pub/Sub; and Dataflow is the sole process that writes to Bronze. Orchestration scheduling and retry logic are covered in the [Orchestration Architecture](#orchestration-architecture) section.
...
plantuml::diagrams/ingestion-architecture.puml\[GCP Ingestion Architecture,align="center",title-align="center",format=svg\]
...
|----|----|
| **Cloud Composer** | Schedules, triggers, and monitors Dataflow ingestion jobs via Airflow DAGs. Supports both cron-based and event-driven triggers. See [Orchestration Architecture](#orchestration-architecture) for details. |
| **Cloud Run** | Executes lightweight scraper and API-fetch logic. Stateless, auto-scaled, and short-lived. Publishes raw events to the `ingest-events` Pub/Sub topic on completion. When a scraper requires external input (e.g., URL lists, pagination cursors, search terms), it subscribes to the `scraper-input` topic to receive those parameters from Dataflow. |
| **Cloud Pub/Sub** | Decoupled, durable message bus with two logical flows: `ingest-events` — Cloud Run publishes raw scraped data; Dataflow subscribes and consumes. `scraper-input` — Dataflow publishes input parameters; Cloud Run subscribes to pick them up. |
| **Cloud Dataflow** | Subscribes to `ingest-events`, applies minimal normalisation (type casting, format detection), and is the **sole writer** to the Bronze layer. Reads existing Bronze records to perform deduplication before committing new data. Publishes to `scraper-input` when a downstream scraper requires parameterised input. |
| **BigQuery and Cloud Storage** | Immutable raw landing zone. Cloud Storage receives unstructured files (PDF, HTML, DOC) in their original format. BigQuery receives structured rows with all original fields preserved. |

- ![cogs](diagrams/icons/cogs.png) **At-least-once delivery**: Pub/Sub guarantees at-least-once; Dataflow uses Bronze read-back to detect and discard duplicate records before writing.

- ![cogs](diagrams/icons/cogs.png) **Parameterised scrapers**: When a scraper requires external input data, Dataflow publishes those parameters to the `scraper-input` topic before or during a run.

- ![cogs](diagrams/icons/cogs.png) **Immutability**: Dataflow never updates or deletes existing Bronze records. Every run appends new date-partitioned files or rows.

- ![cogs](diagrams/icons/cogs.png) **Partitioning**: GCS paths use Hive-style date partitioning (`year=YYYY/month=MM/day=DD/`).BigQuery tables are partitioned by ingestion timestamp.
...
plantuml::diagrams/transformation-architecture.puml\[GCP Transformation Architecture,align="center",title-align="center",format=svg\]
...
| Component | Responsibility |
|----|----|
| **Cloud Composer** | Triggers and monitors Dataflow extraction jobs via Airflow DAGs. Handles scheduling, retries, and alerting for the ETL pipeline. See [Orchestration Architecture](#orchestration-architecture) for details. |
| **Cloud Dataflow** | Reads unstructured files (PDF, HTML, DOC) from Cloud Storage Bronze. Applies OCR, LLM-assisted extraction, NLP entity recognition, and ORM-style mapping to produce structured rows, which it writes to BigQuery Bronze. |
| **Dataform** | Google Cloud’s native SQL transformation orchestrator (analogous to dbt). Manages the DAG of SQL models that move data from Bronze → Silver → Gold inside BigQuery, runs schema and data quality tests after every model, and tracks full column-level lineage. See [Orchestration Architecture](#orchestration-architecture) for details. |
| **BigQuery — Bronze** | Landing zone for structured data produced by both the ingestion pipeline (structured sources) and the Dataflow extraction pipeline (unstructured sources). All rows are append-only and retain original fields plus source metadata. |
| **BigQuery — Silver** | Cleansed and conformed layer. SQL models validate, deduplicate, cast types, conform domains, and handle Slowly Changing Dimensions (SCD). Dataplex enforces data quality rules at this layer. |
| **BigQuery — Gold** | Curated serving layer. SQL models produce business aggregations, cross-domain joins, and pre-computed KPIs. Dataplex maintains the data catalog and column-level lineage. |
| **Cloud Dataplex** | Enforces data quality checks (not-null, referential integrity, range checks) on Silver and Gold. Maintains the data catalog and tracks lineage from Bronze through to Gold. |
...
- ![cogs](diagrams/icons/cogs.png) **Orchestration**: Cloud Composer triggers Dataflow jobs on schedule or on new-file events.

- ![cogs](diagrams/icons/cogs.png) **Techniques**: OCR for scanned PDFs, LLMs for free-text extraction, NLP for entity recognition, ORM-style mappers for semi-structured formats (e.g., HTML tables).

- ![cogs](diagrams/icons/cogs.png) **Immutability**: Dataflow appends extracted rows; raw source files in GCS are never modified.
...
- ![cogs](diagrams/icons/cogs.png) **Bronze → Silver**: validation, deduplication, type standardisation, and SCD handling via SQL.

- ![cogs](diagrams/icons/cogs.png) **Silver → Gold**: business aggregations, cross-domain joins, and pre-computed KPIs via SQL.

- ![cogs](diagrams/icons/cogs.png) **Testing**: Dataform runs schema tests, not-null assertions, and referential integrity checks after every model run.

- ![cogs](diagrams/icons/cogs.png) **Quality & lineage**: Dataplex enforces data quality rules on Silver and Gold and maintains column-level lineage across all layers.

- ![cogs](diagrams/icons/cogs.png) **Idempotency**: All SQL models use `INSERT OVERWRITE` or `MERGE` semantics so any run can be safely repeated without producing duplicates.
...
plantuml::diagrams/orchestration-architecture.puml\[GCP Orchestration Architecture,align="center",title-align="center",format=svg\]
...
| Component | Responsibility |
|----|----|
| **Triggers** | Two kinds: cron schedules (hourly, daily, weekly per source) and event-driven triggers (GCS object-finalise notifications, Pub/Sub messages, BigQuery table update events). Both are configured in Cloud Composer as Airflow sensors or DAG schedule intervals. |
| **Cloud Composer / Airflow** | Outer orchestrator. Runs DAGs for ingestion, unstructured extraction, and the transformation handoff. Handles retries, SLA monitoring, alerting, and dependency chaining across Dataflow and Dataform. |
| **Cloud Dataflow** | Stateless compute jobs launched on demand by Composer. Two job types: `Ingestion Job` — scrapes APIs and writes raw data to Bronze. `Extraction Job` — reads unstructured files from GCS Bronze, applies OCR / LLM / NLP techniques, and writes structured rows to BigQuery Bronze. |
| **Dataform** | Inner SQL orchestrator (analogous to dbt). Triggered by Composer once Bronze is ready. Runs the Bronze → Silver model set (validation, dedup, SCD), then chains into the Silver → Gold model set (aggregations, KPIs). Runs built-in assertion tests after every model and reports failures back to Composer. |
| **BigQuery Silver & Gold** | Output of the Dataform SQL pipeline. Dataform creates or fully refreshes these tables on every successful run using `INSERT OVERWRITE` / `MERGE` semantics. |

- ![cogs](diagrams/icons/cogs.png) **Schedule cadence**: configurable per source — ingest jobs run hourly or daily; extractionand transformation jobs run once ingest completes.

- ![cogs](diagrams/icons/cogs.png) **Event-driven runs**: a GCS `object.finalize` notification or a Pub/Sub message can fire an out-of-cycle DAG run for near-real-time sources.

- ![cogs](diagrams/icons/cogs.png) **Error handling**: Composer retries Dataflow jobs with exponential back-off; failed Dataform assertions quarantine affected partitions and raise an alert without blocking unaffected models.

- ![cogs](diagrams/icons/cogs.png) **Idempotency**: every Dataflow job and every Dataform SQL model can be safely re-run; Bronze appends with deduplication, Silver and Gold use overwrite/merge semantics.

## Monitoring and Alerting Architecture

| Signal | Tool | Action on Failure |
|----|----|----|
| Pipeline run status | Cloud Monitoring + Composer | PagerDuty/email alert to on-call engineer |
| Data quality check failure | Dataplex DQ + Cloud Logging | Quarantine row to bad-data table, alert |
| BigQuery slot utilization | Cloud Monitoring dashboard | Review query optimization |
| Storage growth anomaly | Cloud Monitoring | Alert if daily ingest \> 3x moving average |
| Pipeline SLA breach | Cloud Composer SLA miss sensor | Escalation notification |
...
This chapter explains each Google Cloud service used in the platform, why it was chosen over alternatives, and how it contributes to the overall solution. Non-technical readers can focus on the "business value" callouts in each section.
...
This chapter describes how data is organized, modeled, and governed inside the data lake. Good design here is what turns a "pile of files in the cloud" into a **trusted, reusable data asset** that teams across the organization can rely on. The central question of this chapter is: **how should data be modeled at each layer?** Should Bronze be normalized? Should Silver use Kimball? Should Gold use One Big Table? Each decision has long-term consequences for performance, integrity, and development velocity — and every recommendation here is grounded in the specific objectives of this project.
...
|----|----|----|
| Bronze | `year/month/day` (append only) | Partitioned by `_ingested_at` (ingestion timestamp). External tables used for GCS-resident unstructured files. |
| Silver | `year/month` (reprocessed when source changes) | DATE or TIMESTAMP partition on the business event date (`filing_date`, `_created_at`). Clustering on high-cardinality filter columns. |
| Gold | `year/month` (overwritten on each Dataform run) | DATE partition + clustering on the most common filter columns (e.g., `case_status`, `court_district`, `crm_sync_status`). |
...
Data modeling is the discipline of deciding **how data is structured and related** within each layer. A poor modeling decision in Silver can force every downstream consumer to work around the same structural issues forever. The correct model at each layer depends on the primary consumer, the change frequency of the data, and the relationships between entities in the business domain.
...
|----|----|----|----|
| **Bronze** | Schema-on-Read (raw preservation) | Normalized relational, flat/wide | Normalization in Bronze is premature. Source schemas change; raw fidelity must be preserved for full pipeline replay. |
| **Silver** | Normalized Relational (3NF) | Kimball Star at Silver; One Big Table at Silver | Silver is the integration layer, not the serving layer. 3NF ensures entity integrity and eliminates redundancy across the multi-entity domain of this project. Kimball at Silver conflates integration with serving and forces a single query pattern on all consumers. OBT at Silver creates update anomalies for one-to-many entities. |
| **Gold — Analytics** | Kimball Star Schema | Snowflake Schema; OBT only | Star Schema balances query performance with dimensional flexibility for BI/Looker. Snowflake adds join depth that slows interactive queries without meaningful benefit for this domain’s dimension cardinality. OBT-only loses the ability to filter on well-defined dimension attributes. |

### Bronze Layer: Schema-on-Read (Raw Preservation)

Bronze is **not a modeling layer** — it is an archival layer. The objective is to land data exactly as it was received from the source, augmented only by system-added metadata columns (prefixed with `_`), and nothing else.

> [!IMPORTANT]
> Never apply business transformations in Bronze. No type casting to business types, no field renaming, no normalization, no derivation of new columns. The only permissible additions are ingestion metadata columns prefixed with `_` (e.g., `_ingested_at`, `_source`, `_pipeline_run_id`).

**Why no normalization in Bronze?**

- **Source schemas change.** A court records API may return `case_no` today and `case_number` tomorrow. Raw preservation means Bronze never needs reprocessing for source-side renames.
...
Choosing OBT or Kimball at the Silver layer for this structure creates critical design problems:

| Model at Silver | What Happens | Why This Is Wrong |
|----|----|----|
| **One Big Table** | One row per (case × property × defendant) combination. A case with 2 properties and 3 defendants generates 6 rows, with all case-level fields repeated 6 times. | Update anomalies: a case status change from `ACTIVE` to `SETTLED` requires updating all 6 rows atomically. A missed update produces two conflicting statuses in the same table. |
| **Kimball Star at Silver** | A fact table with grain `case_id × property_id × defendant_id` and denormalized dimension attributes embedded. | Silver is model-agnostic and should serve all consumers. Pre-committing to a Kimball grain at Silver forces every consumer — including ML pipelines and operational APIs — to accept that grain. It also conflates concern: Silver is for integration, Gold is for serving. |
| **Normalized 3NF** | Each entity lives in its own table, deduplicated and linked by foreign keys. Silver rows for `cases` are updated in one place; `properties` and `defendants` reference `case_id` as a foreign key. | This is the correct choice. Silver is a clean, authoritative, entity-oriented representation of the business domain. Gold models then denormalize or aggregate as needed for their specific consumers. |
...
A **Snowflake Schema** further normalizes dimension tables — for example, splitting `dim_properties` into `dim_properties → dim_addresses → dim_zip_codes`. This is appropriate when:

1.  Dimension tables are enormous (tens of millions of rows) and storage is a primary constraint.

2.  Sub-dimensions have deep, independent hierarchies that change at different frequencies (e.g., a product hierarchy with category → subcategory → brand).
...
- Each additional `JOIN` in a Snowflake query increases Looker rendering time and BigQuery slot consumption for every query an analyst runs.
...
All Silver and Gold tables are checked by automated Dataplex DQ tasks after each pipeline run. Failures quarantine the affected records in a `_quarantine` table and trigger an alert before the bad data reaches Gold.

| Check Type | Description | Layer |
|----|----|----|
| Not Null | Critical columns must never be null: `case_id`, `filing_date`, `property_id`, `defendant_id`. | Silver, Gold |
| Uniqueness | Primary key columns must contain unique values per partition. | Silver, Gold |
| Referential Integrity | `case_id` in `properties` and `defendants` must match a valid `case_id` in `cases`. | Silver |
| Value Range | `delinquent_amount > 0`, `assessed_value > 0`, `extraction_confidence BETWEEN 0 AND 1`. | Silver, Gold |
| Freshness | `max(_updated_at)` must be within the expected refresh window after each DAG run. | Gold |
| Row Count Anomaly | Daily row count must not deviate more than 30% from the 7-day moving average. | All |
| PII Token Validation | `defendant_name_token` must never contain a raw name pattern. Checked via regex on Silver and Gold. If a raw name is detected, the pipeline fails and alerts. | Silver, Gold |

## Data Retention Policy

| Layer | Retention Period | Mechanism |
|----|----|----|
| Bronze (raw) | 7 years | GCS Lifecycle rule — transition to COLDLINE after 12 months. BigQuery Bronze tables retained for 7 years via table expiration. |
| Silver (cleansed) | 3 years | BigQuery table expiration — older partitions dropped automatically. |
| Gold (aggregated) | 5 years | BigQuery table expiration; older partitions dropped after 5 years. |
| Bad data / Quarantine | 90 days | GCS Lifecycle delete; retained for investigation and selective replay. |
...
|----|----|----|
| **Acquisitions** | Data Engineering / Legal Operations | `cases`, `properties`, `defendants`, `fact_distressed_acquisitions`, `crm_distressed_export` |
| **Sales / CRM** | Sales Operations | `customers`, `orders`, `opportunities` |
| **Finance** | Finance Team | `transactions`, `invoices`, `cost_centers`, `budgets` |
| **Marketing** | Marketing Team | `campaigns`, `leads`, `channel_performance`, `attribution` |
...
This chapter is aimed at data engineers and cloud architects who need to understand the **how** behind the implementation. It walks through a complete, production-grade end-to-end pipeline for the Distressed Acquisitions use case — from the first DAG that discovers new court cases all the way to the CRM export that enables client outreach. It also covers infrastructure setup, pipeline patterns, testing, and operational practices. **Non-technical stakeholders can skip this chapter.**
...
|----|----|
| **One DAG per concern** | Each DAG has a single responsibility: discover, ingest, extract, or trigger. Limits blast radius — a failing PDF extraction DAG does not block case discovery or property ingestion. |
| **Sensor before extract** | GCS or Pub/Sub sensors wait for source data availability before triggering work. |
| **Idempotent tasks** | All tasks use date-parameterized paths and `WRITE_APPEND` + deduplication (via `QUALIFY ROW_NUMBER()` in Dataform) for safe reruns. |
| **Alerting on failure** | `on_failure_callback` sends to PagerDuty or Slack for every DAG. |
| **No task-level secrets** | All credentials fetched from Secret Manager at runtime. Never stored in DAG code, Airflow Variables, or environment variables. |

## Testing Strategy

| Test Type | Scope | Tool |
|----|----|----|
| Unit tests | Individual transform functions, ORM extraction regex, validation logic | `pytest` + Apache Beam `DirectRunner` |
| Integration tests | End-to-end pipeline on a small synthetic dataset in the `dev` GCP project | Dataflow + pytest + BigQuery test datasets |
| Scraper tests | Mock HTTP responses for the court records API and property GIS API | `pytest` + `responses` library (HTTP mocking) |
| Data quality tests | Automated checks on Silver and Gold tables post-load | Dataplex DQ tasks + Dataform `assertions` blocks |
| Contract tests | Schema compatibility between Pub/Sub message payload and Bronze table schema | Avro schema validation at publish time |
...
|----|----|
| Pipeline runtime parameters (project, region, bucket names) | Airflow Variables (stored encrypted in Composer’s managed Airflow DB) |
| Credentials and secrets (API keys, session cookies) | Google Secret Manager — accessed at runtime, never stored in DAG or scraper code |
| Schema definitions | GCS bucket `gs://{project}-config/schemas/` |
| Data quality rule definitions | YAML files in GCS `gs://{project}-config/dq-rules/` — managed by Dataplex |
| Dataform workspace configuration | Dataform repository in Cloud Source Repositories (linked to Git) |
...
- **Bad Data Investigation** — How to query the `_quarantine` tables and the `extraction_confidence` field in `bronze_acquisitions.defendants_raw` to identify failed PDF extractions. Low-confidence rows are kept in Bronze for manual review and can be re-extracted after updating the ORM pattern.

- **Schema Change Rollout** — Process for evolving a Bronze or Silver schema without breaking downstream consumers. Dataform SQLX models handle additive changes incrementally; backward-incompatible changes require a migration run with `fully_refresh_incremental_tables_enabled: true`.
...
- **PDF Re-extraction** — When the ORM extractor is updated (e.g., improved regex or a new NLP model), historical Bronze PDFs in GCS can be reprocessed by triggering `dag_extract_pdf_to_bronze` with a historical date range and setting Dataform to full refresh for `defendants` and all downstream Gold models.
...
The full pipeline involves four independent DAGs, two Cloud Run scrapers, one Dataflow extraction job, and a Dataform project structured around the [Data Modeling Strategy](#data-modeling-strategy) described in the previous chapter.
...
|----|----|----|----|
| `distressed-cases-new` | DAG 1 | Scraper A (push), Scraper B (push) | `{"case_id": "TX-2025-042", "case_name": "2025-TX-HARRIS-0042", "court_district": "Harris County", "filing_date": "2025-11-15"}` |
| `scraper-property-results` | Scraper A (Cloud Run) | DAG 2 (pull subscription) | `{"case_id": "TX-2025-042", "parcel_id": "1234-AB", "address_raw": "123 Main St, Houston, TX 77002", "assessed_value_raw": "$245,000", "delinquent_amount_raw": "$8,300", …​}` |
| `scraper-pdf-ready` | Scraper B (Cloud Run) | DAG 3 (GCS sensor + Dataflow, subscription for confirmation) | `{"case_id": "TX-2025-042", "gcs_path": "gs://mycompany-bronze-prod/acquisitions/defendants-pdfs/…​/TX-2025-042.pdf"}` |

### Step 1 — DAG 1: Case Discovery

DAG 1 runs nightly. It queries the court records source for cases filed since the last successful run. For each new case, it publishes a Pub/Sub message **and** writes the case metadata directly to `bronze_acquisitions.cases_raw` in the same task — this avoids a separate DAG step for case records and keeps the case row as close as possible in time to the Pub/Sub message that triggers the scrapers.
...
Both scrapers are deployed as independent Cloud Run services. They are registered as Pub/Sub push-subscription endpoints on `distressed-cases-new` and execute **concurrently and independently** — neither waits for the other.

#### Scraper A: Property Information (Structured JSON Output)

This scraper queries a property tax / GIS API using the case name to retrieve structured property records. It publishes results to `scraper-property-results`.
...
This scraper downloads a PDF court-filing document for the given case from the public records portal. It stores the PDF **unmodified** in the GCS Bronze bucket (raw preservation) and publishes a path notification to `scraper-pdf-ready` so that DAG 3 can trigger PDF extraction when it detects new files.
...
DAG 2 runs every 15 minutes to consume structured property results from the `scraper-property-results` Pub/Sub subscription and write them to `bronze_acquisitions.properties_raw`. Messages are acknowledged **only after** a successful BigQuery write to ensure at-least-once delivery semantics.
...
DAG 3 runs hourly. It uses a GCS sensor to detect new PDFs in the Bronze bucket for the current day. When new files are found, it launches a Dataflow job that applies an ORM-style approach — PDF parsing, regex-based entity extraction, and confidence scoring — to produce structured defendant rows and write them to `bronze_acquisitions.defendants_raw`.

> [!NOTE]
> The extraction result is written to BigQuery Bronze — not Silver. The extracted text still carries uncertainty (the `extraction_confidence` score quantifies this). Silver is where validated, deduplicated records live. Bronze absorbs the raw extraction output, including low-confidence rows that Dataplex DQ will flag.
...
The Dataflow job applies an ORM-style mapping: for each PDF, it extracts defendant names and roles using regex patterns and writes one row per defendant found.
...
|----|----|----|
| `bronze_acquisitions.cases_raw` | One row per newly discovered court case. All fields are raw strings exactly as returned by the court records API. | DAG 1 (Python → `bq.insert_rows_json`) |
| `bronze_acquisitions.properties_raw` | One row per property linked to a case. All values are raw strings (e.g., `assessed_value_raw = "$245,000"`). No type casting or address parsing. | DAG 2 (Pub/Sub pull → `bq.insert_rows_json`) |
| `bronze_acquisitions.defendants_raw` | One row per extracted defendant per PDF. Contains the GCS path of the source PDF, the extracted name, role, confidence score, and a 500-character text snippet for debugging. Includes low-confidence placeholder rows where extraction found no match. | DAG 3 (Dataflow ORM → `beam.io.WriteToBigQuery`) |
...
Dataform manages the Bronze → Silver SQL transformation pipeline inside BigQuery. The Silver models clean, validate, cast types, deduplicate records, and tokenize PII to produce the normalized 3NF tables described in [Data Modeling Strategy](#data-modeling-strategy).
...
The last step exports `gold_acquisitions.crm_distressed_export` to the CRM system. This is implemented as a Cloud Run service (or a fifth lightweight Composer DAG) that:

1.  Queries rows where `crm_sync_status = 'NEW'`.

2.  POSTs each row as a prospect record to the CRM REST API.

3.  Updates `crm_sync_status` to `'SYNCED'` on success, or `'FAILED'` on API error, using a BigQuery DML `UPDATE` statement.
...
Security is not an afterthought — it is embedded into every layer of the data lake. This chapter describes the controls in place to protect sensitive data, prevent unauthorized access, and meet regulatory requirements.
...
This chapter captures the most important lessons learned during the project and translates them into clear, actionable guidance for teams operating or extending the data lake. Both business and technical stakeholders will find relevant takeaways here.
...
> [!TIP]
> Every pipeline should be idempotent and able to reprocess any date range on demand. Use date-parameterized paths in GCS and `WRITE_TRUNCATE` for BigQuery loads. This is critical for recovering from bugs, schema errors, and source system issues.
...
This chapter provides a transparent view of the platform’s cloud costs, how costs are structured, and how they compare to the previous state. The data lake’s cost model is **consumption-based** — you pay for what you use, and costs scale with business growth rather than being fixed overhead.
...
The current data lake is Phase 1 of a multi-phase data strategy. This chapter outlines what comes next — from expanding data coverage to enabling AI and machine learning capabilities that can directly drive business outcomes.
